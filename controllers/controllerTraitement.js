...
import Resolution from "../models/traitement.js"
...
*/
    ...
    add = async (req, res) => {
        try {
            ...
            const result = await Resolution.query().insert(req?.body)
    ...
    */
    ...
    show = async (req, res) => {
        try {
            ...
            const data = await Resolution.query()
                .select("affectations.*",
                    "resolutions.commentaire",
                    "users.matricule",
                    "users.nom",
                    "users.prenom",
                    "resolutions.created_at as date_envoie"
                )
                .join("affectations", "affectations.id", "resolutions.affectation_ref")
                .join("users", "affectations.user_ref", "users.matricule")
                .where({ "resolutions.ticket_ref": ticket })
